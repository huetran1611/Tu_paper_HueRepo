...
DEPOT_LON = 105.845
MILE_M = 1609.344
INNER_SIDE_MILE = 20.0
OUTER_SIDE_MILE = 30.0
...
def overpass_query():
    cache_path = REALITY / "overpass_20_30mile_ring_cache.json"
...
def build_osm_pool(existing_keys):
    ...
    for el in overpass_query().get("elements", []):
        ...
        candidates.append({
            ...
            "x": x,
            "y": y,
            "source_dataset": "openstreetmap_20_30_ring",
...
def write_dataset(target_dir, n, m, osm_pool):
    ...
    with (target_dir / f"reality.{new_suffix}.truck_distance_m.txt").open("w", encoding="utf-8", newline="") as f:
        f.write(f"# i j distance_m; {new_suffix} keeps {keep_count} points from {old_suffix} and adds {osm_count} OSM points in 20-30 mile ring\n")
        for i in range(n + 1):
            for j in range(n + 1):
    ...
    fieldnames = [
        ...
        "osrm_snap_distance_m", "source_dataset", "original_id", "inside_30mile_square",
        "outside_20mile_square",
    ]
    with (target_dir / f"reality.{new_suffix}.nodes.csv").open("w", encoding="utf-8", newline="") as f:
        ...
        writer.writerow({
            ...
            "original_id": 0, "inside_30mile_square": "", "outside_20mile_square": "",
        })
        for new_id, p in enumerate(selected, start=1):
            ...
            rec = {
                ...
                "inside_30mile_square": str(in_outer).lower(),
                "outside_20mile_square": str(outside_inner).lower(),
            }
            writer.writerow(rec)
            if p["kind"] == "existing":
            ...
            cust.update({
                ...
                "x_m": p["x"],
                "y_m": p["y"],
                "inside_30mile_square": in_outer,
                "outside_20mile_square": outside_inner,
    ...
    data["meta"].update({
        "description": f"{n}-customer 30new dataset: 75% kept from {old_suffix}, 25% OSM points in 20x20 to 30x30 mile ring",
        "version": new_suffix,
        "source_reality_dataset": f"reality.{old_suffix}",
        "source_reality_dir": str(target_dir),
        "selection_rule": "Remove 1/4 customers from the current 30-mile dataset and replace them with OSM POIs inside the 30x30 mile square but outside the 20x20 mile square.",
        ...
        "random_seed": seed,
        "total_customers": n,
        "kept_customers_from_current_30": keep_count,
        "osm_ring_customers": osm_count,
        "synthetic_customers": 0,
        "truck_distance_rule": (
            "Existing-existing edges are sliced from the current 30-mile matrix; any edge involving an OSM point "
            "uses OSRM table when USE_OSRM_FOR_OSM_EDGES=true, otherwise Euclidean distance multiplied by the "
            "median road/euclidean factor from the current 30-mile matrix."
        ),
        "truck_distance_osm_edges_use_osrm": USE_OSRM_FOR_OSM_EDGES,
        "truck_distance_osm_edges_road_factor": road_factor,
        "vmax_rule": "Existing-existing edges are sliced from the current 30-mile file; any edge involving an OSM point is sampled from the current 30-mile empirical vmax distribution.",
        "theta_rule": "Existing-existing edges are sliced from the current 30-mile file; any edge involving an OSM point uses heavy theta=round(Uniform(0.9,1.0)*base_L[l],2).",
    })
...
def main():
    ...
    for target_dir in TARGET_DIRS:
        for n, m in DATASETS:
            _, rows = read_nodes(target_dir / f"reality.{n}.30.{m}.nodes.csv")
            for row in rows:
