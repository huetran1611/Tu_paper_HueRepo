...
def main():
    ...
    parser.add_argument("--out-dir", default="round2_aggregate")
    parser.add_argument("--instance", default="instance_time_dependent/100.10.1.txt")
    parser.add_argument("--expected-runs", type=int, default=324)
    parser.add_argument("--expected-configs", type=int, default=108)
    ...
    all_runs_path = out_dir / "round2_all_runs.csv"
    ...
    for config_id, config_rows in by_config.items():
        ...
        summary_rows.append({
            ...
            "gamma1": float(first["gamma1"]),
            "gamma2": float(first["gamma2"]),
            "gamma3": float(first["gamma3"]),
            "gamma4": float(first["gamma4"]),
        })

    summary_rows.sort(key=lambda row: (row["mean_objective"], row["std_objective"], -row["feasible_runs"], row["config_id"]))

    summary_path = out_dir / "round2_summary_by_config.csv"
    summary_fields = [
        "rank", "config_id", "runs", "feasible_runs", "mean_objective", "mean_cost",
        "std_objective", "total_runtime_sec", "gamma1", "gamma2", "gamma3", "gamma4",
    ]
    ...
    (out_dir / "round2_best_config.json").write_text(json.dumps(best_json, indent=2) + "\n", encoding="utf-8")
    ...
    fixed_args = [
        ...
        "--h-mode=1",
        "--h-div=4",
        f"--gamma1={best['gamma1']}",
        f"--gamma2={best['gamma2']}",
        f"--gamma3={best['gamma3']}",
        f"--gamma4={best['gamma4']}",
        "--T0=0.5",
        "--alpha=0.998",
        "--kappa=1.0",
        "--tau-v=0.01",
    ...
    (out_dir / "round2_best_for_round3.fixed.args").write_text("\n".join(fixed_args) + "\n", encoding="utf-8")
    ...
    (out_dir / "round2_validation_report.json").write_text(
