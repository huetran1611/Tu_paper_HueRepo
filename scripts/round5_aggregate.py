...
def main():
    ...
    parser.add_argument("--out-dir", default="round5_aggregate")
    ...
    all_runs_path = out_dir / "round5_all_runs.csv"
    ...
    (out_dir / "round5_best_config.json").write_text(json.dumps(best_json, indent=2) + "\n", encoding="utf-8")
    ...
    fixed_args = [
        ...
        "--kappa=1.0",
        "--tau-v=0.005",
        f"--r-destroy={best['r_destroy']}",
    ]
    (out_dir / "round5_final.fixed.args").write_text("\n".join(fixed_args) + "\n", encoding="utf-8")
    ...
    (out_dir / "round5_validation_report.json").write_text(
