param(
    ...
    [string]$Exe = "D:\HueTT\prepare for Phd\Soict_2024_D2d2_extend\Tu_paper_HueRepo\tabubu_TimeDependent_new_instance.exe",
    [int]$Attempts = 1,
    [int]$TimeLimitSec = 0,
    [string]$RunLabel = ""
)
...
foreach ($instance in $instances) {
    ...
    $logPath = Join-Path $instance.FullName "$($prefix)solve_log.txt"
    $solutionPath = Join-Path $instance.FullName "$($prefix)solution_best.txt"
    $iterationPath = Join-Path $instance.FullName "$($prefix)iteration_mode_0.txt"
...
$summaryName = if ([string]::IsNullOrWhiteSpace($RunLabel)) { "solve_summary.csv" } else { "$RunLabel`_solve_summary.csv" }
