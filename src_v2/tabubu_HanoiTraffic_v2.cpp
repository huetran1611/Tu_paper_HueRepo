...
vvd truck_distance_matrix; // OSRM truck distance matrix in meters
vvd drone_distance_matrix; // Euclidean drone distance matrix in meters
...
static string CFG_TRUCK_VIJL_FILE; // optional Hanoi file: i j actual_hour v_ijl_kph
static string CFG_TRUCK_DISTANCE_FILE; // optional Hanoi file: i j distance_m
static string CFG_DRONE_DISTANCE_FILE; // optional Hanoi file: i j distance_m
static string CFG_VEHICLE_CONFIG_FILE; // optional JSON file for truck/drone parameters
...
static bool CFG_HANOI_INPUT = false;
static double CFG_HANOI_DEMAND_MIN = 0.2;
static double CFG_HANOI_DEMAND_MAX = 2.0;
static unsigned CFG_HANOI_DEMAND_SEED = 20260819u;
static double CFG_HANOI_TRUCK_SERVICE_SEC = 60.0;
static double CFG_HANOI_DRONE_SERVICE_SEC = 60.0;
static double CFG_HANOI_DEADLINE_SEC = 3600.0;
...
static const double PENALTY_TARGET_VIOLATION = 0.01; // tau_v
static const double PENALTY_ADAPTATION_RATE = 1.0;   // kappa
...
static bool parse_space_numeric_record(string line, vector<double>& values) {
    values.clear();
...
static string replace_suffix_copy(const string& s, const string& suffix, const string& replacement) {
...
static int csv_column_index(const vector<string>& header, const string& name) {
    string target = lower_copy(name);
    for (int i = 0; i < (int)header.size(); ++i) {
        string col = lower_copy(header[i]);
        if (col == target) return i;
    }
    return -1;
}

static string hanoi_prefix_from_instance_path(const string& filepath) {
    string base = replace_suffix_copy(filepath, "_with_depot.nodes.csv", "");
    if (base == filepath) base = replace_suffix_copy(filepath, "_with_depot.amsts.csv", "");
    return base;
}

static double generated_hanoi_demand(int customer_id) {
    std::mt19937 rng(CFG_HANOI_DEMAND_SEED + 1000003u * (unsigned)customer_id);
    double u = std::generate_canonical<double, 53>(rng);
    return CFG_HANOI_DEMAND_MIN + (CFG_HANOI_DEMAND_MAX - CFG_HANOI_DEMAND_MIN) * u;
}

static double generated_hanoi_deadline(int customer_id) {
    static const double choices[] = {1800.0, 2700.0, 3600.0};
    std::mt19937 rng(CFG_HANOI_DEMAND_SEED + 2000003u * (unsigned)customer_id);
    std::uniform_int_distribution<int> dist(0, 2);
    return choices[dist(rng)];
}

static optional<double> json_number_value(const string& text, const string& key) {
    string quoted_key = "\"" + key + "\"";
    size_t key_pos = text.find(quoted_key);
    if (key_pos == string::npos) return nullopt;
    size_t colon_pos = text.find(':', key_pos + quoted_key.size());
    if (colon_pos == string::npos) return nullopt;
    size_t value_pos = text.find_first_of("-+0123456789.", colon_pos + 1);
    if (value_pos == string::npos) return nullopt;
    ...
    try {
        return stod(text.substr(value_pos, value_end - value_pos));
    } catch (...) {
        return nullopt;
    }
}

static void apply_vehicle_config_json(const string& filepath) {
    if (filepath.empty()) return;
    ifstream fin(filepath);
    if (!fin) {
        cerr << "Error: Cannot open vehicle config JSON " << filepath << endl;
        exit(1);
    }
    string text((istreambuf_iterator<char>(fin)), istreambuf_iterator<char>());
    auto set_double = [&](const string& key, double& target, double scale = 1.0) {
        if (auto value = json_number_value(text, key)) target = max(1e-9, *value * scale);
    };
    auto set_nonnegative = [&](const string& key, double& target) {
        if (auto value = json_number_value(text, key)) target = max(0.0, *value);
    };
    auto set_int = [&](const string& key, int& target) {
        if (auto value = json_number_value(text, key)) target = max(1, (int)llround(*value));
    };

    set_int("truck_count", CFG_OVERRIDE_TRUCKS);
    set_int("drone_count", CFG_OVERRIDE_DRONES);
    set_double("truck_capacity_kg", Dh);
    set_double("drone_capacity_kg", Dd);
    set_double("truck_base_speed_mps", vmax);
    set_double("drone_speed_mps", v_fly_drone);
    set_double("drone_speed_kph", v_fly_drone, 1.0 / 3.6);
    set_double("drone_energy_j", E);
    set_double("drone_takeoff_speed_mps", v_take_off);
    set_double("drone_landing_speed_mps", v_landing);
    set_nonnegative("drone_height_m", height);
    set_nonnegative("drone_power_beta", power_beta);
    set_nonnegative("drone_power_gamma", power_gamma);
    set_nonnegative("truck_service_sec", CFG_HANOI_TRUCK_SERVICE_SEC);
    set_nonnegative("drone_service_sec", CFG_HANOI_DRONE_SERVICE_SEC);
    set_double("deadline_default_sec", CFG_HANOI_DEADLINE_SEC);
}

static void load_distance_matrix_file(const string& filepath, vvd& matrix, const string& label) {
    ifstream fin(filepath);
    if (!fin) {
        cerr << "Error: Cannot open " << label << " distance file " << filepath << endl;
        exit(1);
    }
    matrix.assign(n + 1, vd(n + 1, 0.0));
    string line;
    vector<double> values;
    while (getline(fin, line)) {
        if (!parse_space_numeric_record(line, values)) continue;
...
static void load_hanoi_vijl_file(const string& filepath) {
    if (filepath.empty()) return;
    ifstream fin(filepath);
    if (!fin) {
        cerr << "Error: Cannot open Hanoi v_ijl file " << filepath << endl;
        exit(1);
    }

    const int first_hour = 6;
    const int segment_count = 12; // 6-7, 7-8, ..., 17-18
    truck_vmax_ij.assign(n + 1, vd(n + 1, 1.0));
    truck_theta_ijl.assign(segment_count, vvd(n + 1, vd(n + 1, max(1.0, vmax))));
    time_segments_sigma.assign(segment_count, 1.0);
    ...
    while (getline(fin, line)) {
        ...
        int i = (int)values[0];
        int j = (int)values[1];
        int actual_hour = (int)values[2];
        int l = actual_hour - first_hour;
        double speed_mps = values[3] / 3.6;
        if (i < 0 || i > n || j < 0 || j > n || l < 0 || l >= segment_count || speed_mps <= 1e-8) continue;
        truck_theta_ijl[l][i][j] = speed_mps;
    }
}

void input_hanoi_nodes(string filepath) {
    ifstream fin(filepath);
    if (!fin) {
        cerr << "Error: Cannot open Hanoi nodes file " << filepath << endl;
        exit(1);
    }

    vector<Point> nodes;
    vector<int> node_dronable;
    vector<double> node_demand;
    vector<double> node_drone_service;
    vector<double> node_truck_service;
    vector<double> node_deadline;
    string line;
    bool header_seen = false;
    vector<string> header;
    int col_dronable = -1, col_demand = -1, col_drone_service = -1, col_truck_service = -1, col_lw = -1;
    while (getline(fin, line)) {
        if (line.empty()) continue;
        if (!header_seen) {
            header = split_csv_line(line);
            col_dronable = csv_column_index(header, "Dronable");
            col_demand = csv_column_index(header, "Demand");
            col_drone_service = csv_column_index(header, "Drone_service");
            col_truck_service = csv_column_index(header, "Truck_service");
            col_lw = csv_column_index(header, "Lw");
            header_seen = true;
            continue;
        }
        ...
        nodes[id] = Point(lon, lat, id);
        if ((int)node_dronable.size() <= id) {
            node_dronable.resize(id + 1, -1);
            node_demand.resize(id + 1, -1.0);
            node_drone_service.resize(id + 1, -1.0);
            node_truck_service.resize(id + 1, -1.0);
            node_deadline.resize(id + 1, -1.0);
        }
        if (col_dronable >= 0 && col_dronable < (int)f.size() && !f[col_dronable].empty()) node_dronable[id] = stoi(f[col_dronable]);
        if (col_demand >= 0 && col_demand < (int)f.size() && !f[col_demand].empty()) node_demand[id] = stod(f[col_demand]);
        if (col_drone_service >= 0 && col_drone_service < (int)f.size() && !f[col_drone_service].empty()) node_drone_service[id] = stod(f[col_drone_service]);
        if (col_truck_service >= 0 && col_truck_service < (int)f.size() && !f[col_truck_service].empty()) node_truck_service[id] = stod(f[col_truck_service]);
        if (col_lw >= 0 && col_lw < (int)f.size() && !f[col_lw].empty()) node_deadline[id] = stod(f[col_lw]);
    }

    if (nodes.size() < 2 || nodes[0].id != 0) {
        cerr << "Error: Hanoi nodes file must contain depot id 0 and at least one customer\n";
        exit(1);
    }
    ...
    served_by_drone.assign(n + 1, 1);
    serve_truck.assign(n + 1, CFG_HANOI_TRUCK_SERVICE_SEC);
    serve_drone.assign(n + 1, CFG_HANOI_DRONE_SERVICE_SEC);
    deadline.assign(n + 1, CFG_HANOI_DEADLINE_SEC);
    demand.assign(n + 1, 0.0);
    for (int i = 0; i <= n; ++i) loc[i] = nodes[i];
    served_by_drone[0] = 0;
    serve_truck[0] = serve_drone[0] = deadline[0] = demand[0] = 0.0;
    for (int i = 1; i <= n; ++i) {
        served_by_drone[i] = ((int)node_dronable.size() > i && node_dronable[i] >= 0) ? node_dronable[i] : 1;
        demand[i] = ((int)node_demand.size() > i && node_demand[i] >= 0.0) ? node_demand[i] : generated_hanoi_demand(i);
        serve_drone[i] = ((int)node_drone_service.size() > i && node_drone_service[i] >= 0.0) ? node_drone_service[i] : CFG_HANOI_DRONE_SERVICE_SEC;
        serve_truck[i] = ((int)node_truck_service.size() > i && node_truck_service[i] >= 0.0) ? node_truck_service[i] : CFG_HANOI_TRUCK_SERVICE_SEC;
        deadline[i] = ((int)node_deadline.size() > i && node_deadline[i] > 0.0) ? node_deadline[i] : generated_hanoi_deadline(i);
    }

    string hanoi_prefix = hanoi_prefix_from_instance_path(filepath);
    if (CFG_TRUCK_DISTANCE_FILE.empty()) {
        CFG_TRUCK_DISTANCE_FILE = hanoi_prefix + ".truck_distance_m.txt";
    }
    if (CFG_DRONE_DISTANCE_FILE.empty()) {
        CFG_DRONE_DISTANCE_FILE = hanoi_prefix + ".drone_euclid_distance_m.txt";
    }
    if (CFG_TRUCK_VIJL_FILE.empty()) {
        CFG_TRUCK_VIJL_FILE = hanoi_prefix + ".v_ijl_6h_18h.txt";
    }

    load_distance_matrix_file(CFG_DRONE_DISTANCE_FILE, drone_distance_matrix, "drone");
    load_distance_matrix_file(CFG_TRUCK_DISTANCE_FILE, truck_distance_matrix, "truck");
    distance_matrix = drone_distance_matrix;
    time_segments_sigma.assign(12, 1.0);
}
...
void compute_distance_matrices(const vector<Point>& loc) {
    ...
    if (truck_distance_matrix.empty()) truck_distance_matrix = distance_matrix;
    if (drone_distance_matrix.empty()) drone_distance_matrix = distance_matrix;
}
...
static void configure_time_dependent_truck_speed_model() {
    if (CFG_HANOI_INPUT && !CFG_TRUCK_VIJL_FILE.empty()) {
        load_hanoi_vijl_file(CFG_TRUCK_VIJL_FILE);
        return;
    }
...
static double get_truck_distance(int from, int to) {
    if ((int)truck_distance_matrix.size() == n + 1 &&
        from >= 0 && from <= n &&
        (int)truck_distance_matrix[from].size() == n + 1 &&
        to >= 0 && to <= n) {
        return truck_distance_matrix[from][to];
    }
    return distance_matrix[from][to];
}

static double get_drone_distance(int from, int to) {
    if ((int)drone_distance_matrix.size() == n + 1 &&
        from >= 0 && from <= n &&
        (int)drone_distance_matrix[from].size() == n + 1 &&
        to >= 0 && to <= n) {
        return drone_distance_matrix[from][to];
    }
    return distance_matrix[from][to];
}
...
pair<double, double> compute_truck_route_time(const vi& route, double start=0) {
    ...
    for (int k = 1; k < (int)route.size(); ++k) {
        ...
        double dist_left = get_truck_distance(from, to); // meters
...
pair<double, double> compute_drone_route_energy(const vi& route) {
    ...
    for (int k = 1; k < (int)route.size(); ++k) {
        int from = route[k-1], to = route[k];
        if (from == to) continue;
        double dist = get_drone_distance(from, to); // meters
...
Solution repair_solution_common(Solution sol, const unordered_set<int>& to_destroy) {
    ...
    for (int cust : customers_to_insert) {
        ...
        auto consider_insertion = [&](bool is_truck, int veh_idx, int pos) {
            ...
            if (better_score_with_random_tie(score, best_score, tie_count)) {
                best_score = score;
...
int main(int argc, char* argv[]) {
	    if (argc < 2) {
	        cerr << "Usage: " << argv[0]
	             ...
	             << " [--hanoi] [--truck-distance-file=PATH] [--drone-distance-file=PATH] [--truck-vijl-file=PATH]"
	             << " [--demand-seed=N] [--deadline-sec=SEC] [--truck-service-sec=SEC] [--drone-service-sec=SEC]"
	             << " [--truck-capacity=KG] [--drone-capacity=KG] [--drone-speed-kph=KPH]"
	             << " [--vehicle-config=PATH]"
             << "\n";
        return 1;
    }
    string input_file = argv[1];
	    ...
	    for (int ai = 2; ai < argc; ++ai) {
	        string arg = argv[ai], v;
	        if (parse_kv_flag(arg, "--vehicle-config", v)) {
	            CFG_VEHICLE_CONFIG_FILE = v;
	            break;
	        }
	    }
	    apply_vehicle_config_json(CFG_VEHICLE_CONFIG_FILE);
	    // Parse optional flags
	    for (int ai = 2; ai < argc; ++ai) {
	        string arg = argv[ai];
	        if (arg == "--print-distance-matrix") { print_dist_matrix = true; continue; }
	        if (arg == "--hanoi") { CFG_HANOI_INPUT = true; continue; }
	        string v;
        ...
        if (parse_kv_flag(arg, "--truck-distance-file", v)) { CFG_TRUCK_DISTANCE_FILE = v; continue; }
        if (parse_kv_flag(arg, "--drone-distance-file", v)) { CFG_DRONE_DISTANCE_FILE = v; continue; }
        if (parse_kv_flag(arg, "--truck-vijl-file", v)) { CFG_TRUCK_VIJL_FILE = v; continue; }
        if (parse_kv_flag(arg, "--vehicle-config", v)) { CFG_VEHICLE_CONFIG_FILE = v; continue; }
        if (parse_kv_flag(arg, "--demand-seed", v)) { CFG_HANOI_DEMAND_SEED = (unsigned)stoul(v); continue; }
        if (parse_kv_flag(arg, "--demand-min", v)) { CFG_HANOI_DEMAND_MIN = stod(v); continue; }
        if (parse_kv_flag(arg, "--demand-max", v)) { CFG_HANOI_DEMAND_MAX = stod(v); continue; }
        if (parse_kv_flag(arg, "--deadline-sec", v)) { CFG_HANOI_DEADLINE_SEC = max(1e-9, stod(v)); continue; }
        if (parse_kv_flag(arg, "--truck-service-sec", v)) { CFG_HANOI_TRUCK_SERVICE_SEC = max(0.0, stod(v)); continue; }
        if (parse_kv_flag(arg, "--drone-service-sec", v)) { CFG_HANOI_DRONE_SERVICE_SEC = max(0.0, stod(v)); continue; }
        ...
        if (parse_kv_flag(arg, "--drone-speed-kph", v)) { v_fly_drone = max(1e-9, stod(v) / 3.6); continue; }
        if (arg == "--auto-tune") { auto_tune = true; continue; }
    }

    // Read input instance
    if (input_file.find(".nodes.csv") != string::npos || input_file.find(".amsts.csv") != string::npos) CFG_HANOI_INPUT = true;
    if (CFG_HANOI_INPUT) {
        input_hanoi_nodes(input_file);
    } else {
        input(input_file);
    }
    ...
    if (!CFG_HANOI_INPUT) compute_distance_matrices(loc);
