...
vvd distance_matrix; // compatibility/neighbor distance matrix; uses drone distance by default
vvd truck_distance_matrix; // truck road-distance matrix, loaded from *.truck_distance_m.txt
vvd drone_distance_matrix; // drone distance matrix, loaded from *.drone_distance_m.txt
...
static string CFG_TRUCK_DISTANCE_FILE; // optional file: i j distance_m for truck
static string CFG_DRONE_DISTANCE_FILE; // optional file: i j distance_m for drone
...
static bool is_truck_one_trip_route(const vi& route) {
...
static void normalize_truck_one_trip_route(vi& route) {
    vi cleaned;
    cleaned.reserve(route.size() + 2);
    cleaned.push_back(0);
    for (int node : route) {
        if (node != 0) cleaned.push_back(node);
    }
    if (cleaned.back() != 0) cleaned.push_back(0);
    route.swap(cleaned);
}

static void normalize_truck_routes_one_trip(Solution& sol) {
    for (vi& route : sol.truck_routes) {
        normalize_truck_one_trip_route(route);
    }
}
...
static string companion_file_for(const string& input_file, const string& suffix) {
    filesystem::path p(input_file);
    filesystem::path dir = p.parent_path();
    string name = p.filename().string();
    const string txt_suffix = ".txt";
    if (name.size() >= txt_suffix.size() &&
        name.compare(name.size() - txt_suffix.size(), txt_suffix.size(), txt_suffix) == 0) {
        name.erase(name.size() - txt_suffix.size());
    }
    filesystem::path candidate = dir / (name + suffix);
    return candidate.string();
}

static void set_companion_file_if_present(string& target, const string& input_file, const string& suffix) {
    if (!target.empty()) return;
    string candidate = companion_file_for(input_file, suffix);
    if (filesystem::exists(candidate)) target = candidate;
}
...
void input(string filepath){
    ...
    truck_distance_matrix.assign(n+1, vd(n+1, 0.0));
    drone_distance_matrix.assign(n+1, vd(n+1, 0.0));
...
// Builds Euclidean fallback distance matrices. Hanoi reality runs should load
// truck/drone matrices from companion files after this fallback is initialized.
void compute_distance_matrices(const vector<Point>& loc) {
    int n = loc.size() - 1; // assuming loc[0] is depot
    for (int i = 0; i <= n; ++i) {
        for (int j = 0; j <= n; ++j) {
            double euclid = sqrt((loc[i].x - loc[j].x) * (loc[i].x - loc[j].x)
                                 + (loc[i].y - loc[j].y) * (loc[i].y - loc[j].y));
            distance_matrix[i][j] = euclid;
            truck_distance_matrix[i][j] = euclid;
            drone_distance_matrix[i][j] = euclid;
        }
    }
}

static void load_distance_matrix_file(const string& filepath, vvd& target_matrix, const string& label) {
    ...
    while (getline(fin, line)) {
        ...
        int i = (int)values[0];
        int j = (int)values[1];
        double dist = values[2];
        if (i < 0 || i > n || j < 0 || j > n || dist < -1e-8) continue;
        target_matrix[i][j] = max(0.0, dist);
        ++loaded;
    }
    if (loaded == 0) {
        cerr << "Error: No usable records loaded from " << label << " distance file " << filepath << endl;
        exit(1);
    }
}

static void configure_distance_matrices() {
    ...
    // Keep existing KNN/neighborhood code attached to distance_matrix. Drone
    // distance is the geometric distance surface and is available for every
    // edge in the Hanoi reality instances.
...
pair<double, double> compute_truck_route_time(const vi& route, double start=0) {
    ...
    for (int k = 1; k < (int)route.size(); ++k) {
        ...
        double dist_left = truck_distance_matrix[from][to]; // meters
...
pair<double, double> compute_drone_route_energy(const vi& route) {
    ...
    for (int k = 1; k < (int)route.size(); ++k) {
        int from = route[k-1], to = route[k];
        if (from == to) continue;
        double dist = drone_distance_matrix[from][to]; // meters
...
vector<double> check_truck_route_feasibility(const vi& route, double start=0) {
    ...
    // Check capacity for a single truck trip. Depot visits inside the route are
    // not allowed, so capacity is not reset before the route ends.
    double capacity_violation = 0.0;
    double total_demand = 0.0;
    if (!is_truck_one_trip_route(route)) {
        capacity_violation += 1e6;
    }
    for (int k = 1; k < (int)route.size(); ++k) {
...
Solution generate_initial_solution(uint64_t seed = UINT64_MAX){
    ...
    // Finally, enforce one depot-to-depot trip per truck and ensure all routes end at depot.
    normalize_truck_routes_one_trip(sol);
    for (int i = 0; i < h; ++i) {
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        cerr << "Usage: " << argv[0]
             ...
             << " [--truck-distance-file=PATH] [--drone-distance-file=PATH]"
    ...
    set_companion_file_if_present(CFG_TRUCK_DISTANCE_FILE, input_file, ".truck_distance_m.txt");
    set_companion_file_if_present(CFG_DRONE_DISTANCE_FILE, input_file, ".drone_distance_m.txt");
    set_companion_file_if_present(CFG_TRUCK_VMAX_FILE, input_file, ".vmax_ij.txt");
    set_companion_file_if_present(CFG_TRUCK_THETA_FILE, input_file, ".theta_ijl.txt");
