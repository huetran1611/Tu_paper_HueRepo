...
static void convert_truck_speed_model_to_static_average() {
    int segment_count = truck_time_segment_count();
    if (segment_count <= 0) return;

    vvd static_vmax(n + 1, vd(n + 1, vmax));
    for (int i = 0; i <= n; ++i) {
        for (int j = 0; j <= n; ++j) {
            if (i == j) {
                static_vmax[i][j] = 0.0;
            ...
            for (int l = 0; l < segment_count; ++l) {
                double theta = 1.0;
                if (l < (int)truck_theta_ijl.size() &&
                    i < (int)truck_theta_ijl[l].size() &&
                    j < (int)truck_theta_ijl[l][i].size()) {
                    theta = truck_theta_ijl[l][i][j];
                }
                sum_speed += truck_vmax_ij[i][j] * theta;
            }
            static_vmax[i][j] = sum_speed / segment_count;
        }
    }

    truck_vmax_ij = static_vmax;
    truck_theta_ijl.assign(segment_count, vvd(n + 1, vd(n + 1, 1.0)));
}
...
static void configure_time_dependent_truck_speed_model() {
    ...
    convert_truck_speed_model_to_static_average();
    cout << "Static truck speed model: vmax_ij = average_l(vmax_ij * theta_ijl), theta_ijl = 1\n";
}
...
Solution generate_initial_solution(uint64_t seed = UINT64_MAX){
    ...
    vd service_times_drone(d, 0.0); // service times for each drone
    vd capacity_used_truck(h, 0); // capacity used by each truck
    vd capacity_used_drone(d, 0); // capacity used by each drone
    vd timebomb_truck(h, 1e18); // timebomb for each truck
    vd timebomb_drone(d, 1e18); // timebomb for each drone
    vd energy_used_drone(d, 0); // energy used by each drone
    ...
    vector<char> active_truck(h, 1), active_drone(d, 1);
...
int main(int argc, char* argv[]) {
    ...
    int available_by_drone = 0;
    for (int i = 1; i <= n; ++i) {
        if (served_by_drone[i]) ++available_by_drone;
    }
    cout << "Drone-available customers after capacity/energy/deadline filter: "
         << available_by_drone << "/" << n << " ("
         << fixed << setprecision(2) << (100.0 * available_by_drone / max(1, n)) << "%)\n";
