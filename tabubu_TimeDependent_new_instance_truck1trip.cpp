...
double v_fly_drone = 60.0 * 1000.0 / 3600.0, v_take_off = 30.0 * 1000.0 / 3600.0, v_landing = 15.0 * 1000.0 / 3600.0; // drone speeds (m/s)
...
double power_beta = 66.0, power_gamma = 397.0; // coefficients for drone energy consumption per second
...
static vi normalized_truck_one_trip_route(vi route) {
...
static Solution enforce_truck_one_trip_solution(Solution sol) {
...
static string find_first_existing_path(const vector<filesystem::path>& candidates) {
    for (const auto& p : candidates) {
        if (filesystem::exists(p)) return p.string();
    }
    return "";
}

static void resolve_new_instance_paths(string& input_file) {
    filesystem::path p(input_file);
    if (!filesystem::is_directory(p)) return;

    string instance_name = p.filename().string();
    string solver_input = find_first_existing_path({
        p / (instance_name + "_solver.txt"),
        p / "solver.txt",
        p / "customers_solver.txt"
    });
    if (solver_input.empty()) {
        cerr << "Error: Cannot find solver input in instance directory " << p.string() << endl;
        cerr << "Expected " << instance_name << "_solver.txt, solver.txt, or customers_solver.txt" << endl;
        exit(1);
    }
    input_file = solver_input;

    if (CFG_TRUCK_VMAX_FILE.empty()) {
        CFG_TRUCK_VMAX_FILE = find_first_existing_path({
            p / (instance_name + "_solver.vmax_ij.txt"),
            p / "v_ij.txt",
            p / "vmax_ij.txt"
        });
    }
    if (CFG_TRUCK_THETA_FILE.empty()) {
        CFG_TRUCK_THETA_FILE = find_first_existing_path({
            p / (instance_name + "_solver.theta_ijl.txt"),
            p / "theta_ij.txt",
            p / "theta_ijl.txt"
        });
    }

    cout << "Resolved new-instance directory:\n";
    cout << "  solver input: " << input_file << "\n";
    cout << "  truck vmax: " << (CFG_TRUCK_VMAX_FILE.empty() ? "(default)" : CFG_TRUCK_VMAX_FILE) << "\n";
    cout << "  truck theta: " << (CFG_TRUCK_THETA_FILE.empty() ? "(default)" : CFG_TRUCK_THETA_FILE) << "\n";
}
...
vector<double> check_truck_route_feasibility(const vi& route, double start=0) {
    vi one_trip_route = normalized_truck_one_trip_route(route);
    // Check deadlines
    auto [time, deadline_violation] = compute_truck_route_time(one_trip_route, start);
    
    // Check capacity for one truck trip. A truck route may only be
    // depot -> customers -> depot, so capacity is not reset mid-route.
    double capacity_violation = 0.0;
    double total_demand = 0.0;
    for (int k = 1; k < (int)one_trip_route.size(); ++k) {
        int customer = one_trip_route[k];
        if (customer == 0) {
            continue;
        } else {
...
Solution greedy_insert_customer(Solution sol, int customer, bool minimize_delta) {
    ...
    auto try_insert = [&](vi base_route, bool is_truck, int route_idx) {
        if (!is_truck && !served_by_drone[customer]) return;
...
Solution generate_initial_solution(uint64_t seed = UINT64_MAX){
    ...
    auto has_unvisited_drone_unable = [&]() {
        for (int cust = 1; cust <= n; ++cust) {
            if (!visited[cust] && !served_by_drone[cust]) return true;
        }
        return false;
    };
    auto customer_priority_order = [&](const vector<int>& source) {
        vi ordered;
        ordered.reserve(source.size());
        for (int cust : source) {
            if (cust >= 1 && cust <= n && !served_by_drone[cust]) ordered.push_back(cust);
        }
        for (int cust : source) {
            if (cust >= 1 && cust <= n && served_by_drone[cust]) ordered.push_back(cust);
        }
    ...
    while (num_of_visited_customers < n && iter > 0) {
        ...
        int active_count = 0;
        bool prioritize_drone_unable = has_unvisited_drone_unable();
        for (int i = 0; i < h; ++i) {
        ...
        for (int i = 0; i < h; ++i) { // consider paired drone index i
            if (prioritize_drone_unable) continue;
        ...
        if (is_truck) {
            ...
            vi ordered_customers;
            ordered_customers.reserve(n);
            for (int cust = 1; cust <= n; ++cust) {
                if (!served_by_drone[cust]) ordered_customers.push_back(cust);
            }
            for (int cust = 1; cust <= n; ++cust) {
                if (served_by_drone[cust]) ordered_customers.push_back(cust);
            }
            for (int cust : ordered_customers) {
                if (visited[cust]) continue;
                if (prioritize_drone_unable && served_by_drone[cust]) continue;
            ...
            if (!assigned) {
                ...
                if (current_node2 != 0) {
                    ...
                    r.push_back(0);
                    sol.truck_routes[truck_idx] = r;
                    active_truck[truck_idx] = 0; // one-trip truck is finished after returning to depot
                    made_progress = true;
                }
                else {
                    active_truck[truck_idx] = 0;
    ...
    vi fallback_order;
    fallback_order.reserve(n);
    for (int cust = 1; cust <= n; ++cust) {
        if (!served_by_drone[cust]) fallback_order.push_back(cust);
    }
    for (int cust = 1; cust <= n; ++cust) {
        if (served_by_drone[cust]) fallback_order.push_back(cust);
    }
    for (int cust : fallback_order) {
        if (visited[cust]) continue;
...
Solution local_search(const Solution& initial_solution, int neighbor_id, int current_iter, double best_cost, double (*solution_cost)(const Solution&)) {
    ...
    if (neighbor_id == 0) {
        ...
        return enforce_truck_one_trip_solution(best_neighbor);
        ...
        auto consider_relocate_pair = [&](const vi& base_route, bool is_truck_mode, int critical_vehicle_id) {
            ...
            auto apply_candidate = [&](const vi& crit_route, const vd& crit_metrics_local,
                ...
                return enforce_truck_one_trip_solution(candidate);
            };
...
/*              cout.setf(std::ios::fixed);
       ...
       auto [critical_idx, crit_is_truck] = critical_solution_index(initial_solution);
        ...
        if (best_neighbor_cost_local + 1e-8 < best_neighbor_cost) {
            return enforce_truck_one_trip_solution(best_candidate_neighbor);
        }
        return enforce_truck_one_trip_solution(initial_solution);
    }
// ...existing code...
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        cerr << "Usage: " << argv[0]
             << " input_file_or_instance_dir [--print-distance-matrix]"
    ...
    resolve_new_instance_paths(input_file);
