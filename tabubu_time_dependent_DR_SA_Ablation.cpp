...
struct ModeSelectionConfig {
    ...
    DestroyStrategy destroy_strategy = DestroyStrategy::None;
    double destroy_rate = 0.0;
    bool use_sa = false;
    double sa_initial_temperature = 0.0;
    double sa_cooling_rate = 1.0;
};

static string CFG_ABLATION_CONFIG = "all";
static double CFG_EXTENDED_FIXED_MULTIPLIER = 2.0;
static string CFG_ABLATION_PREFIX = "ablation_dr_sa";
static int CFG_SEED_BASE = 42;
static double CFG_DESTROY_RATE = 0.3;
...
Solution destroy_worst_repair_random(Solution sol) {
    unordered_set<int> to_destroy;
    int destroy_count = max(1, static_cast<int>(n * CFG_DESTROY_RATE));
...
Solution tabu_search(const Solution& initial_solution, int num_initial_sol, vector<double>& iter_current, vector<double>& iter_best, vector<bool>& iter_feasible, const ModeSelectionConfig& mode_config) {
    ...
    double sa_temperature = max(1e-12, mode_config.sa_initial_temperature);
    ...
    while (iter < total_iters) {
        ...
        if (neighbor_score + 1e-12 < best_solution_score_now) {
            ...
            if (mode_config.use_sa) {
                double delta = neighbor_score - current_score;
                double accept_prob = exp(-delta / max(sa_temperature, 1e-12));
        ...
        if (mode_config.use_sa) {
            sa_temperature *= mode_config.sa_cooling_rate;
        }
        ...
        if (iter % CFG_MAX_ITER_PER_SEGMENT == 0) {
            ...
            if (no_improve_segments % 4 == 0 && no_improve_segments > 0) {
                if (mode_config.destroy_strategy == DestroyStrategy::Worst) {
                    current_sol = destroy_worst_repair_random(current_sol);
                    current_score = score_solution_for_mode(current_sol, scoring_mode_iter);
                    destroy_repair_count++;
                    cout << "No improvement for " << no_improve_segments
                         << " segments; applying worst destroy-repair r_d="
                         << mode_config.destroy_rate << ". New makespan: "
                         << current_sol.total_makespan << "\n";
                } else if (mode_config.destroy_strategy == DestroyStrategy::Random) {
                    current_sol = destroy_random_repair_random(current_sol);
                    current_score = score_solution_for_mode(current_sol, scoring_mode_iter);
                    destroy_repair_count++;
                    cout << "No improvement for " << no_improve_segments
                         << " segments; applying random destroy-repair r_d="
                         << mode_config.destroy_rate << ". New makespan: "
                         << current_sol.total_makespan << "\n";
                } else {
                    cout << "No improvement for " << no_improve_segments
                         << " segments; destroy-repair disabled for ablation.\n";
                }
                tabu_list_10.clear();
                tabu_list_11.clear();
                tabu_list_20.clear();
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        cerr << "Usage: " << argv[0]
             ...
             << " [--config=all|dr_random_0_1|dr_random_0_2|dr_random_0_3|dr_worst_0_1|dr_worst_0_2|dr_worst_0_3|sa_T100_a0_998|sa_T100_a0_9998|sa_T150_a0_998|sa_T150_a0_9998|both_worst_0_2_T150_a0_9998]"
             << " [--ablation-prefix=NAME]"
             << " [--seed-base=N]"
             << "\n";
        return 1;
    }
    ...
    for (int ai = 2; ai < argc; ++ai) {
        ...
        if (parse_kv_flag(arg, "--config", v)) { CFG_ABLATION_CONFIG = v; continue; }
        if (parse_kv_flag(arg, "--strategy", v)) { CFG_ABLATION_CONFIG = v; continue; }
    ...
    auto add_config = [&](const string& key,
                          const string& label,
                          DestroyStrategy destroy_strategy,
                          double destroy_rate,
                          bool use_sa,
        ...
        configs.push_back({
            key,
            label,
            ModeSelectionStrategy::SwitchingObjective,
            1.0,
            destroy_strategy,
            destroy_rate,
            use_sa,
    ...
    auto add_all_configs = [&]() {
        add_config("dr_random_0_1", "Random, r_d=0.1", DestroyStrategy::Random, 0.1, false, 0.0, 1.0);
        add_config("dr_random_0_2", "Random, r_d=0.2", DestroyStrategy::Random, 0.2, false, 0.0, 1.0);
        add_config("dr_random_0_3", "Random, r_d=0.3", DestroyStrategy::Random, 0.3, false, 0.0, 1.0);
        add_config("dr_worst_0_1", "Worst, r_d=0.1", DestroyStrategy::Worst, 0.1, false, 0.0, 1.0);
        add_config("dr_worst_0_2", "Worst, r_d=0.2", DestroyStrategy::Worst, 0.2, false, 0.0, 1.0);
        add_config("dr_worst_0_3", "Worst, r_d=0.3", DestroyStrategy::Worst, 0.3, false, 0.0, 1.0);
        add_config("sa_T100_a0_998", "(T0,alpha)=(100,0.998)", DestroyStrategy::None, 0.0, true, 100.0, 0.998);
        add_config("sa_T100_a0_9998", "(T0,alpha)=(100,0.9998)", DestroyStrategy::None, 0.0, true, 100.0, 0.9998);
        add_config("sa_T150_a0_998", "(T0,alpha)=(150,0.998)", DestroyStrategy::None, 0.0, true, 150.0, 0.998);
        add_config("sa_T150_a0_9998", "(T0,alpha)=(150,0.9998)", DestroyStrategy::None, 0.0, true, 150.0, 0.9998);
        add_config("both_worst_0_2_T150_a0_9998", "Worst, r_d=0.2; (T0,alpha)=(150,0.9998)", DestroyStrategy::Worst, 0.2, true, 150.0, 0.9998);
    };

    if (CFG_ABLATION_CONFIG == "all") {
        add_all_configs();
    } else if (CFG_ABLATION_CONFIG == "dr_random_0_1") {
        add_config("dr_random_0_1", "Random, r_d=0.1", DestroyStrategy::Random, 0.1, false, 0.0, 1.0);
    } else if (CFG_ABLATION_CONFIG == "dr_random_0_2") {
        add_config("dr_random_0_2", "Random, r_d=0.2", DestroyStrategy::Random, 0.2, false, 0.0, 1.0);
    } else if (CFG_ABLATION_CONFIG == "dr_random_0_3") {
        add_config("dr_random_0_3", "Random, r_d=0.3", DestroyStrategy::Random, 0.3, false, 0.0, 1.0);
    } else if (CFG_ABLATION_CONFIG == "dr_worst_0_1") {
        add_config("dr_worst_0_1", "Worst, r_d=0.1", DestroyStrategy::Worst, 0.1, false, 0.0, 1.0);
    } else if (CFG_ABLATION_CONFIG == "dr_worst_0_2") {
        add_config("dr_worst_0_2", "Worst, r_d=0.2", DestroyStrategy::Worst, 0.2, false, 0.0, 1.0);
    } else if (CFG_ABLATION_CONFIG == "dr_worst_0_3") {
        add_config("dr_worst_0_3", "Worst, r_d=0.3", DestroyStrategy::Worst, 0.3, false, 0.0, 1.0);
    } else if (CFG_ABLATION_CONFIG == "sa_T100_a0_998") {
        add_config("sa_T100_a0_998", "(T0,alpha)=(100,0.998)", DestroyStrategy::None, 0.0, true, 100.0, 0.998);
    } else if (CFG_ABLATION_CONFIG == "sa_T100_a0_9998") {
        add_config("sa_T100_a0_9998", "(T0,alpha)=(100,0.9998)", DestroyStrategy::None, 0.0, true, 100.0, 0.9998);
    } else if (CFG_ABLATION_CONFIG == "sa_T150_a0_998") {
        add_config("sa_T150_a0_998", "(T0,alpha)=(150,0.998)", DestroyStrategy::None, 0.0, true, 150.0, 0.998);
    } else if (CFG_ABLATION_CONFIG == "sa_T150_a0_9998") {
        add_config("sa_T150_a0_9998", "(T0,alpha)=(150,0.9998)", DestroyStrategy::None, 0.0, true, 150.0, 0.9998);
    } else if (CFG_ABLATION_CONFIG == "both_worst_0_2_T150_a0_9998") {
        add_config("both_worst_0_2_T150_a0_9998", "Worst, r_d=0.2; (T0,alpha)=(150,0.9998)", DestroyStrategy::Worst, 0.2, true, 150.0, 0.9998);
    } else {
        cerr << "Unknown --config value: " << CFG_ABLATION_CONFIG << "\n";
        return 1;
    }
    ...
    auto run_config = [&](const ModeSelectionConfig& config) {
        ...
        CFG_DESTROY_RATE = config.destroy_rate;
